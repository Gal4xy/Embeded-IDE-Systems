...
public class AddGlobalsActionDelegate extends ActionDelegate implements IViewActionDelegate, ISelectionListener, IPartListener {
	...
	class SortedListSelectionDialog extends ListSelectionDialog {
		...
		protected Control createDialogArea( Composite parent ) {
			//{modified by maobaolong at 2013-03-11
			...
			text.addModifyListener(new ModifyListener(){
				...
				public void modifyText(ModifyEvent e) {
					...
					List<IGlobalVariableDescriptor> globalsList = new ArrayList<IGlobalVariableDescriptor>();
					for(IGlobalVariableDescriptor g:fGlobals){
						if(g.getName().startsWith(str)){
							globalsList.add(g);
						}
					}
					getViewer().setInput(globalsList.toArray(new IGlobalVariableDescriptor[globalsList.size()]));
					getViewer().refresh();
				}});
			//}modified by maobaolong at 2013-03-11
			return da;
